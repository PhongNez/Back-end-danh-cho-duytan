...
let searchProduct = async (req, res) => {
  try {
    ...
    let [search] = await pool.execute(
      "select  product.*,  price_list.price from product_price JOIN  product ON product_price.id_product = product.id_product JOIN price_list ON product_price.id_pricelist = price_list.id_pricelist where  price_list.status = 1 and name_product like ?",
      ["%" + name + "%"]
    );
    return res.status(200).json({
      message: search,
