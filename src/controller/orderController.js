...
let orderHistory = async (req, res) => {
    try {
        ...
        let [response] = await pool.execute('SELECT *,a.price as gia FROM order_detail a,orders b,product c WHERE a.id_order =b.id_order and a.id_product =c.id_product and id_account=? and b.status=?', [id_account, status]);
        return res.status(200).json({
            listOrder: response
        })
    } catch (e) {
...
let orderAccount = async (req, res) => {
    try {
        let id_account = req.params.id_account;
        console.log(id_account);
        let [response] = await pool.execute('SELECT * FROM orders WHERE id_account=?', [id_account]);
        return res.status(200).json({
            listOrder: response
        })
    } catch (e) {
...
let updateStatusPrice = async (req, res) => {
    try {
        let { status } = req.body
        let { id } = req.query
        let [response] = await pool.execute('UPDATE price_list SET status = ? WHERE id_pricelist = ?', [status, id])
        return res.status(200).json({
            message: 'Thành công'
        })
    } catch (e) {
