  ...
  return new Promise(async (resolve, reject) => {
    try {
      let [data] = await pool.execute(
        "select c.id_product,c.quantity,p.name,p.price,p.images,c.size,c.price_size from cart c,product p where c.id_account=? and c.id_product =p.id_product",
        [id_account]
      );
      //console.log(data);
      resolve(data);
...
let addCart = (id_account, id_product, price_size, quantity, size) => {
  return new Promise(async (resolve, reject) => {
    try {
      let add = await pool.execute(
        "insert into cart (id_account, id_product,price_size, quantity,size) values (?,?,?,?,?)",
