...
let createNewProduct = async (req, res) => {
  ...
  try {
    let { name, detail, id_pricelist, id_category, id_tax } = req.body;
    console.log("Phong body:", req.body);
    console.log("Req.file: ", req.file);

    console.log(name, detail, id_pricelist, id_category, id_tax);
    let images = req.file.filename;
    console.log('Price list: ', id_pricelist);
    //bắt lỗi trống thông tin
    if (!name || !detail || !images || !id_category || !id_tax || !id_pricelist) {
      return res.status(200).json({
    ...
    let insert = await pool.execute(
      "insert into product (id_category,name_product, detail, images,id_tax,status) values (?,?,?,?,?,1)",
      [id_category, name, detail, images, id_tax]
    );
    console.log('insert: ', insert[0].insertId);
    // insert[0].insertId
    //insert bảng product_price
    if (insert && insert[0] && insert[0].insertId) {
      let insert2 = await pool.execute(
        "insert into product_price (id_product,id_pricelist, size) values (?,?,1)",
        [insert[0].insertId, id_pricelist]
      );
    }
    return res.status(200).json({
    ...
    if (id === "ALL") {
      query = `
                SELECT 
                    ...
                    category.name,
                    price_list.price,
                    CAST(size.size AS CHAR) AS size
                FROM 
                    product_price
                JOIN 
                    product ON product_price.id_product = product.id_product
                JOIN 
                    category ON product.id_category = category.id_category
                JOIN 
                    price_list ON product_price.id_pricelist = price_list.id_pricelist
                JOIN 
                    size ON product_price.size = size.id_size
                WHERE 
                    price_list.status = 1;
            `;
      [response] = await pool.execute(query);
    } else {
      query = `
                SELECT 
                    product.*, 
                ...
                JOIN 
                    size ON product_price.size = size.id_size
                WHERE 
                    product.id_category = ? AND price_list.status = 1;
            `;
      [response] = await pool.execute(query, [id]);
    }
    // console.log(response);
...
let getListTax = async (req, res) => {
  try {
    let [tax] = await pool.execute('select * from tax')
    return res.status(200).json({ listTax: tax });
  } catch (e) {
    return res.status(200).json({
      listTax: tax
    })
...
let getListPrice = async (req, res) => {
  try {
    let [price] = await pool.execute('select * from price_list')
    return res.status(200).json({ listPrice: price });
  } catch (e) {
    return res.status(200).json({
      listPrice: price
    })
...
let getProductPhong = async (req, res) => {
  ...
  try {
    ...
    if (!page) {
      [response] = await pool.execute(
        // "select *,p.name as name_product from product p,category c where p.id_category=c.id_category"
        "select * from product p,category c where p.id_category=c.id_category"
      );
    } else {
      [response] = await pool.execute(
        // "select *,p.name as name_product from product p,category c where p.id_category=c.id_category"
