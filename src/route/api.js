...
const initAPIRoute = (app) => {
  ...
  //API load thuế
  router.get(
    "/tax/get", APIController.getListTax
  );

  //API load giá
  router.get(
    "/price/get", APIController.getListPrice
  );

  //API lịch sử đơn đặt hàng
  router.get('/account/lichsudathang/:id_account/:status', OrderController.orderHistory)
  router.get('/account/donhangtheotaikhoan/:id_account', OrderController.orderAccount)

  //Api update trạng thái bảng giá 
  router.put('/admin/update-price', OrderController.updateStatusPrice)
